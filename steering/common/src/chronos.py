...
from core.perturb import identity
import logging
import argparse
from chronos import ChronosPipeline


def get_Chronos_nnsight(device="cpu"):
...
def get_activations_Chronos(
    ...
    samples
        ...
        (batch_size, num_samples, prediction_length).
    """
    chronos_model, chronos_tokenizer = get_Chronos_nnsight(device=device)
    context_tensor = torch.tensor(context, dtype=torch.float32, device=device)
...
def perturb_activations_Chronos(
    ...
    model_outputs_numpy = prediction_final.detach().numpy()

...
METHOD = 'median'
PLOT_SAVE_PATH = f"real_world_normal_to_abnormal_{METHOD}.pdf"
LAYER_TO_PLOT = 17
...
steering_normal_to_ab = get_steering_matrix(normal_activations, abnormal_activations, method=METHOD)
...
one_to_other_steering_matrix = steering_normal_to_ab
...
steering_normal_to_ab.shape


# In[ ]:


single_normal = test_data[test_labels == 0][5].reshape(1, 1, -1)
print(single_normal.shape)
...
non_perturbed_output, perturbed_output = simple_perturbation(single_normal, steering_normal_to_ab, space='reconstruction') 
...
multiple_normal_samples = test_data[test_labels == 0]
normal_embedded = []
perturbed_embedded = []
for i in range(30):
    non_perturbed_output, perturbed_output = simple_perturbation(multiple_normal_samples[i].reshape(1, 1, -1), steering_normal_to_ab, space='embedding')
    normal_embedded.append(non_perturbed_output)
    perturbed_embedded.append(perturbed_output)

normal_embedded = np.array(normal_embedded)
...
embedded_test = moment.embed(x_enc=torch.tensor(multiple_normal_samples[0].reshape(1,1,-1), device=DEVICE,dtype=torch.float32)).embeddings.detach().cpu().numpy()
embedded_test
...
y_hats_normal = svm_classifier.predict(normal_embedded)
y_hats_perturbed = svm_classifier.predict(perturbed_embedded)

print("Normal Accuracy: ", accuracy_score(np.zeros(15), y_hats_normal))
print("Perturbed Accuracy: ", accuracy_score(np.zeros(15), y_hats_perturbed))
...
swaps = np.sum(y_hats_normal != y_hats_perturbed)
